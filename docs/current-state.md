...
- **pisquad CLI**（已发布 `@nicklin/pisquad@0.3.1`）：npm 全局包，提供 `install` / `upgrade` / `version` / `help`；upgrade 含 docs 在内的 sha256 diff + tar.gz 备份；self-update 前比较 registry 版本，已是最新则跳过
