...
- [ ] `src/upgrade/self.ts`：
  - `selfUpdate({ logger })`：检测当前 `pisquad` 是否来自全局 npm（`which pisquad` 命中且路径在 npm 全局目录下）；若是 → 跑 `npm i -g @nicklin/pisquad@latest`
...
- [ ] 不引入 npm 锁文件污染用户全局（`npm i -g @nicklin/pisquad@latest` 不写本仓库锁文件）
